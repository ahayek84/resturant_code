...
class HomeController extends Controller{
	...
	public function viewMobileHomePageToOne($request, $response, $args){
		$id = $args['id'];
		$restaurant = Restaurant::where('restaurant_id', $id)->first();
		return $this->view->render($response, 'mobilerestuarant.php', array('restaurant' => $restaurant));
	}

	public function aboutusInformation($request, $response, $args){
        $id = $args['id'];
		if(!$this->auth->check()){
	...
	public function editAboutusInformation($request, $response, $args){
        $id = $args['id'];
